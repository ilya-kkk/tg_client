...
- `app/config.py` loads environment configuration and session paths.
- `sessions/` stores Telethon session files at runtime (gitignored).
- `docker-compose.yml` defines local services (`tg_api`, optional `db`, `adminer`, `n8n`, `proxy` profiles).
...
- Run full stack (API + DB + n8n + adminer): `docker compose --profile all up --build`
...
- Before opening a PR, at minimum run local startup and smoke-test key endpoints (`/`, `/auth/login`, `/chats`).
...
- Never commit `.env`, API credentials, or session files.
