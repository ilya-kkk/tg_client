...
- [ ] **Обработать основные ошибки и ограничения**:
...
- [ ] **Обрабатывать включение/выключение через тумблер** — при `PATCH is_active=false` кампания перестаёт участвовать в минутном цикле без перезапуска приложения.

### 4. Frontend: страница «Нейрокомментарии»

- [ ] **Создать страницу** `FrontEnd/app/(dashboard)/ai-commenting/page.tsx`:
